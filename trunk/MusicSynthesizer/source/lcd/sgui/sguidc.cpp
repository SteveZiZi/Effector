...
C_SGUIDC::C_SGUIDC(C_WNDBASE *pWnd):C_SGUIOBJ(C_SGUIOBJ::OBJ_T_DC)
{
	if (pWnd == NULL)
	{//screen dc
		m_Rect.x = 0; m_Rect.w = SGUI_LcdWidth();
		m_Rect.y = 0; m_Rect.h = SGUI_LcdHeight();
	}
	else
	{
	...
	m_pWnd  = pWnd;
	m_pBMP  = &SGUI_LcdScrn;
	m_pBMP->SetBmpInfo(SGUI_LcdWidth(), SGUI_LcdHeight(), SGUI_LcdBpp());
...
C_SGUIOBJ *C_SGUIDC::GetStockGuiObj(S_BYTE ObjName)
{
	...
	static C_SGUISYM16x16 SGUI_SYM16x16;
	static C_SGUISYM08x16 SGUI_SYM08x16;
	...
	case SGUI_OBJ_E5x8:
		return &SGUI_E5x8;
	case SGUI_OBJ_SYM16x16:
		return &SGUI_SYM16x16;
	case SGUI_OBJ_SYM08x16:
		return &SGUI_SYM08x16;
	default:
		break;
	}
	return NULL;
...
bool C_SGUIDC::FlushScreen(void)
{
	...
	if (m_pWnd == NULL)
	{
		SGUI_flushScreen(0, 0, SGUI_LcdWidth(), SGUI_LcdHeight());
	}
	else
	{
