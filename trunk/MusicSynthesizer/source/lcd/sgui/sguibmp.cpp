...
void C_SGUIBMP::SetBmpInfo(S_WORD w, S_WORD h, S_WORD bpp)
{
	m_Width  = w;
	m_Height = h;
	m_BPP    = bpp;
...
void C_SGUIBMP::SetBmpData(void *pBuff)
{
	m_pBuff = pBuff;
}

void *C_SGUIBMP::GetBmpData(void)
{
	return m_pBuff;
}


C_SGUISCRN::C_SGUISCRN(void):C_SGUIBMP(160, 160, 1, SGUI_UsrBuf)
{
}
...
int C_SGUISCRN::FillHLine(S_WORD x, S_WORD y, S_WORD w, int color)
{
	...
	S_WORD X;
	S_BYTE *p = (S_BYTE*)m_pBuff + y * SGUI_LcdLine() + (x >> 3);
