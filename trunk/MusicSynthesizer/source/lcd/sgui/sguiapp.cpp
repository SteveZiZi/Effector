...
int C_SGUIAPP::SetGuiDriver(int GuiDriverID)
{
	SGUI_SetDriver(GuiDriverID);
	return 0;
}

int C_SGUIAPP::SysInit(const char *pResourcePath)
{
	SGUI_ResourcePathInit(pResourcePath);
	if (SGUI_LcdInit() != 0)
		return -1;
...
int C_SGUIAPP::TranslateMsg(S_GUIMSG &msg)
{
	...
	case GM_TIME:
		if (m_pCurCaret != NULL)
			return m_pCurCaret->SendWndMsg(msg.Msg, msg.wParam, msg.lParam);
	default:
		break;
	}
	return 0;
