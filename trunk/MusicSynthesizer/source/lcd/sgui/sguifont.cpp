...
extern char g_SGUI_ResourcePath[CFG_PATH_FILE_MAX_LEN];
...
C_SGUIF12x12::C_SGUIF12x12(void)
{
	...
	sprintf(FontName, "%s%s", g_SGUI_ResourcePath, FONT12X12);
...
{
	char FontName[CFG_PATH_FILE_MAX_LEN];
	sprintf(FontName, "%s%s", g_SGUI_ResourcePath, FONT16X16);
	m_fpFont = fopen(FontName, "rb");
}
...
/** @brief symbol 16x16
 */
C_SGUISYM16x16::C_SGUISYM16x16(void)
{
	char FontName[CFG_PATH_FILE_MAX_LEN];
	sprintf(FontName, "%s%s", g_SGUI_ResourcePath, SYM16X16);
	m_fpFont = fopen(FontName, "rb");
}

C_SGUISYM16x16::~C_SGUISYM16x16()
{
	if (m_fpFont)
		fclose(m_fpFont);
}

int C_SGUISYM16x16::GetWidth(S_WORD font)
{
	return 16;
}

int C_SGUISYM16x16::GetHeight(S_WORD font)
{
	return 16;
}

int C_SGUISYM16x16::DrawChar(S_WORD x, S_WORD y, char ch, C_SGUIBMP *pBMP)
{
	S_BYTE font, mod[32];
	memset(mod, 0x00, sizeof(mod));
	GetFontMold(ch, mod);
...
int C_SGUISYM16x16::DrawFont(S_WORD x, S_WORD y, S_WORD ch, C_SGUIBMP *pBMP)
{
	return DrawChar(x, y, (char)ch, pBMP);
}

int C_SGUISYM16x16::GetFontMold(S_WORD font, S_BYTE *pFontBuf)
{
	if (m_fpFont == NULL)
		return -1;
	if (font >= 0x20 && font < 0x7F)
...
/** @brief symbol 08x16
 */
C_SGUISYM08x16::C_SGUISYM08x16(void)
{
	char FontName[CFG_PATH_FILE_MAX_LEN];
	sprintf(FontName, "%s%s", g_SGUI_ResourcePath, SYM08X16);
	m_fpFont = fopen(FontName, "rb");
}

C_SGUISYM08x16::~C_SGUISYM08x16()
{
	if (m_fpFont)
		fclose(m_fpFont);
}

int C_SGUISYM08x16::GetWidth(S_WORD font)
{
	return 8;
}

int C_SGUISYM08x16::GetHeight(S_WORD font)
{
	return 16;
}

int C_SGUISYM08x16::DrawChar(S_WORD x, S_WORD y, char ch, C_SGUIBMP *pBMP)
{
	S_BYTE font, mod[16];
	memset(mod, 0x00, sizeof(mod));
	GetFontMold(ch, mod);
...
int C_SGUISYM08x16::DrawFont(S_WORD x, S_WORD y, S_WORD ch, C_SGUIBMP *pBMP)
{
	return DrawChar(x, y, (char)ch, pBMP);
}

int C_SGUISYM08x16::GetFontMold(S_WORD font, S_BYTE *pFontBuf)
{
	if (m_fpFont == NULL)
		return -1;
	if (font >= 0x20 && font < 0x7F)
