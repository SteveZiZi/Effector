 ...
 *  @return void
 */
void bitset_PrintString(string &strBitset, int from, int to, char delimit, char field);
...
public:
	...
	/** @brief 将from-to区间置1 */
	void SetBit(int from, int to){for (; from <= to; from++) set(from);}
	/** @brief 按位串描述置位
	 *  @param[in] strBitset 位串，如"1-3", "11001"
	 *  @param[in] delimit 分隔符，如'-', 0
	 */
	void SetBit(const string &strBitset, char delimit)
	{
		if (delimit == 0) SetBitNoDelimit(strBitset);
		else SetBitWithDelimit(strBitset, delimit);
	}
	/** @brief 分析参数串
	 ...
	 *  @return void
	 */
	void ParseString(const string &strParam, char delimit = '-', char field = ',')
	{
		strings strField;
		if (strField.split(strParam, field) != 0)
		{
			for(size_t i = 0; i < strField.size(); i++)
				SetBit(strField[i], delimit);
		}
	}
	/** @brief 输出格式化串：如1-3,5-8
	 *  @param[out] strBitset 数出缓冲
	 ...
	 *  @return void
	 */
	void PrintString(string &strBitset, char delimit = '-', char field = ',')const
	{
		if (delimit == 0) PrintStringNoDelimit(strBitset);
		else PrintStringWithDelimit(strBitset, delimit, field);
	}
	/** @brief 分析参数串
...
protected:
	...
	{
		for (size_t i = 0; i<strBitset.size() && i<size(); i++)
			if (strBitset[i] == '1') set(i);
	}
	/** @brief 按串信息(含连接符)设置位 */
	void SetBitWithDelimit(const string &strBitset, char delimit)
	{
		strings strField;
		if (strField.split(strBitset, delimit) > 0)
		{
			int from = atoi(strField[0].c_str()), to = from;
			if (strField.size() >= 2) to = atoi(strField[1].c_str());
			if (from < 0) from = 0;//容错
			if (to < 0 || to >= (int)size()) to = (int)size() - 1;//容错
			SetBit(from, to);
		}
	}
	/** @brief 输出位信息串 */
	void PrintStringNoDelimit(string &strBitset)const
	{
		for (size_t i = 0; i < size(); i++)
		{
			if (test(i)) strBitset.push_back('1');
			else strBitset.push_back('0');
		}
	}
	/** @brief 输出位信息串(含连接符) */
	void PrintStringWithDelimit(string &strBitset, char delimit, char field)const
	{
		int from = -1, to = -1;
		for (size_t i = 0; i < size(); i++)
		{
			...
			{
				bitset_PrintString(strBitset, from, to, delimit, field);
				from = -1, to = -1;
			}
		}
		if (from != -1 && to != -1)
